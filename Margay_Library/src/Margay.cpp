...
int Margay::begin(uint8_t *Vals, uint8_t numVals, String header_)
{
	...
	for (int i = EEPROMLen - 8; i < EEPROMLen; i++)
	...
	delay(100);
	if (Serial.available())
	{ // If time setting info available
	...
	{																	 // If using Model > v2.2, use PCINT for log button
		*digitalPinToPCMSK(LogInt) |= bit(digitalPinToPCMSKbit(LogInt)); // enable pin
	...
	if (BatError)
	{ // Battery voltage is below level where hardware functionality can be gaurenteed
		for (int i = 0; i < 10; i++)
		{
			LED_Color(RED);
			delay(100);
	...
	{ // Battery charge % is at a concerning level, recomend repacing batteries
		for (int i = 0; i < 10; i++)
		{
...
void Margay::SDtest()
{
	...
	if (SDError && !CardNotPresent)
		Serial.println("FAIL"); // If card is inserted and still does not connect propperly, throw error
	else if (!SDError && !CardNotPresent)
		Serial.println("PASS"); // If card is inserted AND connectects propely return success
}
...
void Margay::clockTest()
{
	...
	if (YearNow == 00)
	{ // If value is 2000, work around Y2K bug by setting time to Jan 1st, midnight, 2049
...
void Margay::batTest()
{
	if (getBatVoltage() < BatVoltageError)
		BatError = true; // Set error flag if below min voltage
	if (getBatPer() < BatPercentageWarning)
		BatWarning = true; // Set warning flag is below set percentage
	Serial.print("Bat = ");
...
float Margay::getBatPer()
{
	...
	if (Per < 0)
		return 0; // Do not allow return of non-sensical values
	if (Per > 100)
		return 100; // Is this appropriate? Float voltage could be higher than specified and still be correct
	return Per;
}
...
String Margay::getOnBoardVals()
{
	...
	if (Model < MODEL_2v0)
		...
		return LogTimeDate + "," + String(EnviroSense.GetString()) + String(RTCTemp) + "," + String(BatVoltage) + ",";
}
...
void Margay::run(String (*update)(void), unsigned long logInterval) // Pass in function which returns string of data
{
	...
	{								   // Catch alarm if not reset properly
...
void Margay::addDataPoint(String (*update)(void)) // Reads new data and writes data to SD
{
	String data = "";
	if (Model >= MODEL_2v0)
		EnviroSense.begin(0x77); // Re-initialize BME280  //FIX??
...
void Margay::powerAux(bool State)
{
	...
	if (State)
		powerOB(ON); // Turn on on-board power if required
	if (Model >= MODEL_2v0)
	{									 // use positive logic for Model v2.0 and newer
